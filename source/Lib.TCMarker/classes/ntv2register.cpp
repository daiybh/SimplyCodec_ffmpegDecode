...
{
	return WriteRegister(kRegEveryFrameTaskFilter, (ULWord)mode);
}


bool CNTV2Card::GetEveryFrameServices (NTV2EveryFrameTaskMode & outMode)
{
	return ReadRegister (kRegEveryFrameTaskFilter, reinterpret_cast <ULWord *> (&outMode));
}
...
{
	return ReadRegister (kRegDefaultVideoOutMode, &outMode);
}
	...
	if(!(standard == NTV2VideoFormatStandards[value] && frameRate == NTV2VideoFormatFrameRates[value] && keepVancSettings))
	{
		...
		if (NTV2_IS_QUAD_FRAME_FORMAT(value))
		{
			if (channel < NTV2_CHANNEL5)
			{
				...
				WriteRegister(kRegVideoFormatCh1, value);
				WriteRegister(kRegVideoFormatCh2, value);
				WriteRegister(kRegVideoFormatCh3, value);
				WriteRegister(kRegVideoFormatCh4, value);
			}
			else
			{
				...
				WriteRegister(kRegVideoFormatCh5, value);
				WriteRegister(kRegVideoFormatCh6, value);
				WriteRegister(kRegVideoFormatCh7, value);
				WriteRegister(kRegVideoFormatCh8, value);
			}
		}
		else
		{
			...
			{
				WriteRegister (kRegVideoFormatCh1 + channel, value);
			}
			else
			{
				for (ULWord i = 0; i < ::NTV2DeviceGetNumVideoChannels (GetDeviceID ()); i++)
				{
					WriteRegister (kRegVideoFormatCh1 + i, value);
				}
			}
		}
	}
...
{
	return WriteRegister (kRegProgressivePicture, value);
}
...
{
	ULWord	returnVal	(0);
	bool	result1		(ReadRegister (kRegProgressivePicture, &returnVal));
...
bool CNTV2Card::GetReference (NTV2ReferenceSource & outValue)
{
	...
	bool CNTV2Card::SetReferenceSource (NTV2ReferenceSource value, bool ajaRetail)
	{
		...
		if (ajaRetail == true)
		{
			...
			return WriteRegister(kDisplayReferenceSelect, refSelect);
		}
		else
...
bool CNTV2Card::GetProgramStatus(SSC_GET_FIRMWARE_PROGRESS_STRUCT *statusStruct)
{
	...
	ReadRegister(kRegFlashSize, &totalSize);
	ReadRegister(kRegFlashStatus, &totalProgress);
	ReadRegister(kRegFlashState, &state);
...
bool CNTV2Card::ProgramMainFlash(const char *fileName)
{
	...
	if(::NTV2DeviceHasSPIFlash(_boardID))
	{
		...
		WriteRegister(kRegFlashSize, programSize);
		WriteRegister(kRegFlashStatus, 0);
	}
	else
	{
		delete bitfileBuffer;
	...
	//erase main
	WriteRegister(kRegFlashState, kProgramStateEraseMainFlashBlock);
	bool didErase = EraseFlashBlock(numSectors, sectorSize);

	if (didErase == true)
	{
		//program the main bitfile
		WriteRegister(kRegFlashState, kProgramStateProgramFlash);
		...
		for(uint32_t blockCount = 0; blockCount < twoFiftySixBlockSizeCount; blockCount++, baseAddress += 256)
		{
			...
			_programStatus += 256;
			WriteRegister(kRegFlashStatus, _programStatus);
		}
		...
		WriteRegister(kRegFlashStatus, programSize);
		WriteRegister(kRegFlashState, kProgramStateFinished);
	}
...
bool CNTV2Card::VerifyMainFlash(const char *fileName)
{
	...
	WriteRegister(kRegFlashState, kProgramStateVerifyFlash);
	ULWord errorCount = 0;
...
bool CNTV2Card::ReadSDIInVPID (const NTV2Channel inChannel, ULWord & outValue_A, ULWord & outValue_B)
{
	...
	bool CNTV2Card::ReadUartRxFifoSize (ULWord * pOutSize)				{return pOutSize ? ReadRegister (kRegUartRxFifoSize, pOutSize) : false;}
#endif	//	!defined (NTV2_DEPRECATE)


bool CNTV2Card::GetPCIDeviceID (ULWord & outPCIDeviceID)				{return ReadRegister (kRegPCIDeviceID, &outPCIDeviceID);}
...
bool CNTV2Card::GetColorSpaceMatrixSelect (NTV2ColorSpaceMatrixType* type, NTV2Channel channel)
{
	...
	bool CNTV2Card::UpdateK2ColorSpaceMatrixSelect(NTV2VideoFormat currFormat, bool ajaRetail)
	{
		...
		if (ajaRetail == true)
		{
			...
			ReadRegister(kColorSpaceMode, (ULWord *)&cscType);
			...
			ReadRegister(kRGB10Range, &cscRange);
	...
	bool CNTV2Card::UpdateK2LUTSelect(NTV2VideoFormat currFormat, bool ajaRetail)
	{
		...
		if (ajaRetail == true)
		{
			...
			ReadRegister(kGammaMode, (ULWord *)&gammaType);
			...
			ReadRegister(kK2RegRGBRangeConverterLUTType, (ULWord *)&rgbConverterLUTType);	
			...
			ReadRegister(kLUTType, (ULWord *)&currLUT);
			...
			if (wantedLUT != NTV2_LUTCustom && currLUT != wantedLUT)
			{
				...
				WriteRegister(kLUTType, wantedLUT);
			}
		}
...
{
	bool bResult = WriteRegister(kK2RegSecondaryFormatSelect, format);
...
{
	return ReadRegister (kK2RegSecondaryFormatSelect, reinterpret_cast <ULWord *> (&outFormat));
}


bool CNTV2Card::SetInputVideoSelect (NTV2InputVideoSelect input)
{
	bool bResult = WriteRegister(kK2RegInputSelect, input);
...
{
	return ReadRegister(kK2RegInputSelect, reinterpret_cast <ULWord *> (&outInputSelect));
}
...
bool CNTV2Card::GetSDIInput3GbPresent (bool & outValue, const NTV2Channel channel)
{
	...
	//	printf ("BitfileSwitchNeeded:\n");
		...
		ReadRegister(kRegDebug1,&debugRegValue);
...
bool CNTV2Card::WriteSDProcAmpControlsInitialized	(ULWord value)		{return WriteRegister (kRegProcAmpSDRegsInitialized,		value);}
bool CNTV2Card::WriteSDBrightnessAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpStandardDefBrightness,	value);}
bool CNTV2Card::WriteSDContrastAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpStandardDefContrast,		value);}
bool CNTV2Card::WriteSDSaturationAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpStandardDefSaturation,	value);}
bool CNTV2Card::WriteSDHueAdjustment				(ULWord value)		{return WriteRegister (kRegProcAmpStandardDefHue,			value);}
bool CNTV2Card::WriteSDCbOffsetAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpStandardDefCbOffset,		value);}
bool CNTV2Card::WriteSDCrOffsetAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpStandardDefCrOffset,		value);}
bool CNTV2Card::WriteHDProcAmpControlsInitialized	(ULWord value)		{return WriteRegister (kRegProcAmpHDRegsInitialized,		value);}
bool CNTV2Card::WriteHDBrightnessAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpHighDefBrightness,		value);}
bool CNTV2Card::WriteHDContrastAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpHighDefContrast,			value);}
bool CNTV2Card::WriteHDSaturationAdjustmentCb		(ULWord value)		{return WriteRegister (kRegProcAmpHighDefSaturationCb,		value);}
bool CNTV2Card::WriteHDSaturationAdjustmentCr		(ULWord value)		{return WriteRegister (kRegProcAmpHighDefSaturationCr,		value);}
bool CNTV2Card::WriteHDCbOffsetAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpHighDefCbOffset,			value);}
bool CNTV2Card::WriteHDCrOffsetAdjustment			(ULWord value)		{return WriteRegister (kRegProcAmpHighDefCrOffset,			value);}
bool CNTV2Card::ReadSDProcAmpControlsInitialized	(ULWord *value)		{return ReadRegister (kRegProcAmpSDRegsInitialized,			value);}
bool CNTV2Card::ReadSDBrightnessAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpStandardDefBrightness,		value);}
bool CNTV2Card::ReadSDContrastAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpStandardDefContrast,		value);}
bool CNTV2Card::ReadSDSaturationAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpStandardDefSaturation,		value);}
bool CNTV2Card::ReadSDHueAdjustment					(ULWord *value)		{return ReadRegister (kRegProcAmpStandardDefHue,			value);}
bool CNTV2Card::ReadSDCbOffsetAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpStandardDefCbOffset,		value);}
bool CNTV2Card::ReadSDCrOffsetAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpStandardDefCrOffset,		value);}
bool CNTV2Card::ReadHDProcAmpControlsInitialized	(ULWord *value)		{return ReadRegister (kRegProcAmpHDRegsInitialized,			value);}
bool CNTV2Card::ReadHDBrightnessAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpHighDefBrightness,			value);}
bool CNTV2Card::ReadHDContrastAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpHighDefContrast,			value);}
bool CNTV2Card::ReadHDSaturationAdjustmentCb		(ULWord *value)		{return ReadRegister (kRegProcAmpHighDefSaturationCb,		value);}
bool CNTV2Card::ReadHDSaturationAdjustmentCr		(ULWord *value)		{return ReadRegister (kRegProcAmpHighDefSaturationCr,		value);}
bool CNTV2Card::ReadHDCbOffsetAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpHighDefCbOffset,			value);}
bool CNTV2Card::ReadHDCrOffsetAdjustment			(ULWord *value)		{return ReadRegister (kRegProcAmpHighDefCrOffset,			value);}
...
{
	ULWord val = 0;
	ReadRegister(kRegMailBoxAcquire, &val);
	return val;
}
...
{
	ULWord val = 0;
	ReadRegister(kRegMailBoxRelease, &val);
	return val;
}
...
{
	ULWord val = 0;
	ReadRegister(kRegMailBoxAbort, &val);
	return val;
}
