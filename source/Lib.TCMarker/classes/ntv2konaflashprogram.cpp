...
void CNTV2KonaFlashProgram::EraseBlock(FlashBlockID blockID)
{
	if (IsOpen ())
	{
        ...
        WriteRegister(kRegFlashSize,numSectors);
		for (uint32_t sectorCount = 0; sectorCount < numSectors; sectorCount++ )
		{
            WriteRegister(kRegFlashStatus,sectorCount);
        ...
        WriteRegister(kRegFlashStatus,numSectors);
		if(!_bQuiet)
...
bool CNTV2KonaFlashProgram::ProgramFromMCS(bool verify)
{
	...
	if (IsOpen())
	{
		...
		{
            WriteRegister(kRegFlashState,kProgramStateCalculating);
            WriteRegister(kRegFlashSize,MCS_STEPS);
            WriteRegister(kRegFlashStatus,NextMcsStep());
			...
			else if (basePartitionAddress >= 0x0100 && basePartitionAddress < 0x0200)
			{
                ...
                WriteRegister(kRegFlashState, kProgramStateErasePackageInfo);
				EraseBlock(MCS_INFO_BLOCK);
                WriteRegister(kRegFlashState, kProgramStateProgramPackageInfo);
			}
			else if (basePartitionAddress >= 0x0200 && basePartitionAddress < 0x0400)
			{
				if (!hasErasedSOCs)
				{
                    ...
                    WriteRegister(kRegFlashState, kProgramStateEraseBank3);
					EraseBlock(SOC1_FLASHBLOCK);
					printf("Erase SOC Bank 2\n");
                    WriteRegister(kRegFlashState, kProgramStateEraseBank4);
				...
				if (basePartitionAddress >= 0x0200 && basePartitionAddress < 0x0300)
				{
                    ...
                    WriteRegister(kRegFlashState, kProgramStateProgramBank3);
				}
				else
				{
					blockID = SOC2_FLASHBLOCK;
					linearOffsetToBankOffset = 0x0300;
                    WriteRegister(kRegFlashState, kProgramStateProgramBank4);
				}
			}
			else
			{
            ...
            WriteRegister(kRegFlashSize, totalBlockCount);
			for (uint32_t blockCount = 0; blockCount < totalBlockCount; blockCount++)
			{
                WriteRegister(kRegFlashStatus,blockCount);
            ...
            if (verify)
			{
                ...
                case SOC1_FLASHBLOCK:
                    WriteRegister(kRegFlashState,kProgramStateVerifyBank3);
                    break;
                case SOC2_FLASHBLOCK:
                    WriteRegister(kRegFlashState,kProgramStateVerifyBank4);
                    break;
                case MCS_INFO_BLOCK:
                    WriteRegister(kRegFlashState,kProgramStateVerifyPackageInfo);
                    break;
                }
...
bool CNTV2KonaFlashProgram::VerifySOCPartition(FlashBlockID flashID, uint32_t flashBlockOffset)
{
	...
	int32_t percentComplete = 0;
	uint32_t bufferIndex = 0;
    WriteRegister(kRegFlashSize,dwordsPerPartition);
	for (uint32_t dwordCount = 0; dwordCount < dwordsPerPartition; dwordCount += 100)//dwordCount++)
	{
        WriteRegister(kRegFlashStatus,dwordCount);
		WriteRegister(kRegXenaxFlashAddress, baseAddress);
		WriteRegister(kRegXenaxFlashControlStatus, READFAST_COMMAND);
		WaitForFlashNOTBusy();
		uint32_t flashValue;
