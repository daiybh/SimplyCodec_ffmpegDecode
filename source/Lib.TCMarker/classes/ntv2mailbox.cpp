...
CNTV2MailBox::CNTV2MailBox(CNTV2Card & device) : mDevice( device )
{
    ...
    baddr = SAREK_MAILBOX;   // base address
...
bool CNTV2MailBox::writeMB(uint32_t val, uint32_t timeout)
{
    ...
    if (rv)
    {
        WriteAddress(baddr + MB_WRDATA,val);
    }
    return rv;
}
...
bool CNTV2MailBox::readMB(uint32_t & val, uint32_t timeout)
{
    ...
    if (rv)
    {
         ReadAddress(baddr + MB_RDDATA, &val);
    }
    return rv;
}
...
{
    uint32_t val;
    ReadAddress(baddr + MB_STATUS,&val);
    return val;
}

bool CNTV2MailBox::ReadAddress(const ULWord inAddress, ULWord & outValue, const ULWord inMask, const ULWord inShift)
{
   bool rv = mDevice.ReadRegister(inAddress/4, &outValue, inMask, inShift);
   return rv;
}

bool CNTV2MailBox::WriteAddress(ULWord address, ULWord value, ULWord mask, ULWord shift)
{
   bool rv = mDevice.WriteRegister(address/4, value, mask, shift);
   return rv;
}


bool CNTV2MailBox::ReadChannelAddress (const ULWord inAddress, ULWord & outValue, const ULWord inMask, const ULWord inShift)
{
    NTV2RegInfo	regInfo	(inAddress / 4, 0, inMask, inShift);
    bool rv = mDevice.BankSelectReadRegister (NTV2RegInfo (chanAddress / 4, chanNumber | chanPS), regInfo);
    if (rv)
        outValue = regInfo.registerValue;
    return rv;
}

bool CNTV2MailBox::WriteChannelAddress(ULWord address, ULWord value, ULWord mask, ULWord shift)
{
    return mDevice.BankSelectWriteRegister (NTV2RegInfo (chanAddress / 4, chanNumber | chanPS), NTV2RegInfo (address/4, value, mask, shift));
}

void CNTV2MailBox::SetChannel(ULWord channelAddress, ULWord channelNumber, ULWord channelPS)
{
    chanAddress = channelAddress;
