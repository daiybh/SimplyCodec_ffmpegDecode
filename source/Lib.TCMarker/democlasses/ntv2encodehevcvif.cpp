...
void NTV2EncodeHEVCVif::SetupAutoCirculate (void)
{
    ...
    mInputTransferStruct.frameBufferFormat		= mCapturePixelFormat;
...
void NTV2EncodeHEVCVif::VideoInputWorker (void)
{
	...
	device.StartAutoCirculate (mInputTransferStruct.channelSpec);

	while (!mGlobalQuit)
	{
		AUTOCIRCULATE_STATUS_STRUCT	acStatus;
		device.GetAutoCirculate (mInputTransferStruct.channelSpec, &acStatus);
		...
		if (acStatus.state == NTV2_AUTOCIRCULATE_RUNNING && acStatus.bufferLevel > 1)
		{
            ...
            if (pVideoData)
            {
                ...
                if (mWithAudio)
                {
					mInputTransferStruct.audioBuffer		= pVideoData->pAudioBuffer;
					mInputTransferStruct.audioBufferSize	= pVideoData->audioBufferSize;
                }

                // do the transfer from the device into our host AvaDataBuffer...
                device.TransferWithAutoCirculate (&mInputTransferStruct, &mInputTransferStatusStruct);

                // get the video data size
                pVideoData->videoDataSize = pVideoData->videoBufferSize;
                pVideoData->audioDataSize = 0;
				pVideoData->frameTime = mInputTransferStatusStruct.frameStamp.frameTime;
                pVideoData->lastFrame = mLastFrame;

                if (mWithAudio)
                {
                    // get the audio data size
                    pVideoData->audioDataSize = mInputTransferStatusStruct.audioBufferSize;
                }
	...
	// Stop AutoCirculate...
	device.StopAutoCirculate (mInputTransferStruct.channelSpec);
